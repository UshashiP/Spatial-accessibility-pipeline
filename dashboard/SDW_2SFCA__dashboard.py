...
def main() -> None:
    ...
    run_requested = st.button("Run SDW-2SFCA", type="primary", use_container_width=True)
    ...
    if run_requested:
        ...
        if cached is not None and float(supply_multiplier) == 1.0:
            ...
            if live_mode:
                ...
                with st.spinner("Running SDW-2SFCA (this may take a few minutes)..."):
    ...
    if saved_state is None:
        st.info("Select inputs and click **Run SDW-2SFCA**.")
        return

    if saved_state["signature"] != current_signature and not run_requested:
        st.info("Parameters changed. Click Run SDW-2SFCA to refresh.")
